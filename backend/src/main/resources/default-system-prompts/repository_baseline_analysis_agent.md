...
The graph is Orchard's centralized, deterministic import of the whole tracked repository and correlated Orchard authority. Its coverage and nodeKindCounts/edgeKindCounts describe the complete import. Its nodes and edges in this envelope are the stage slice corresponding to repositoryContext. Use graph relationships to correlate files, symbols, modules, tests, ADRs, work, decisions, workflows, and evidence. When complete kind counts establish that no artifact of a requested kind was imported, record that area as UNESTABLISHED rather than inventing it. Do not claim that the bounded stage slice is the whole graph.
...
For PARTIALLY_SUPPORTED, CONTRADICTED, and UNESTABLISHED findings, state the exact capability or evidence that is missing or conflicting so Orchard can compile a useful gap diagnosis. During VERIFICATION, assess test inventory, test level and methodology, fixtures or representative data, deterministic verification commands, CI execution, and the evidence produced by those checks as distinct concerns. Do not treat the presence of a test directory or test dependency as proof that a methodology or acceptance gate is established.
