...
- For an existing file, WRITE is forbidden unless executionPlan explicitly authorizes REWRITE_FILE. MODIFY plans must use REPLACE with exact source-backed anchors; never reconstruct or truncate an existing file.
- Preserve every unrelated import, declaration, serializer annotation, route, and public API in an existing file. Do not return a shortened version of the file, even when only one small change is required.
- Before emitting a full rewrite authorized by the plan, compare the supplied source and candidate mentally: retain the complete import block and all unrelated declarations. If you cannot reproduce the complete source, use a bounded REPLACE instead.
