...
internal fun RepositoryAnalysisAttemptStore.compileRetainedExactPathOperations(
    ...
    val retained = load().asReversed().asSequence()
        ...
        .distinctBy { it.path }
        .toMutableList()
    if (output.operations.none { it.action == PLAN_OPERATION_CREATE } &&
        output.scopeCoverage.any { it.scope.trim().startsWith("Backend ", ignoreCase = true) }
    ) {
        load().asReversed().asSequence()
            .filter { it.runId == runId && it.baseRevision == baseRevision }
            .mapNotNull { it.rejectedPlan }
            .flatMap { it.operations.asSequence() }
            .firstOrNull { it.action == PLAN_OPERATION_CREATE && it.path !in currentPaths }
            ?.let(retained::add)
    }
    val sourceOperations = (output.operations.filter { it.action != PLAN_OPERATION_VERIFY } + retained)
        .take(MAX_SOURCE_OPERATIONS_PER_PLAN)
    val operations = sourceOperations + output.operations.filter { it.action == PLAN_OPERATION_VERIFY }
    val orderMap = operations.mapIndexed { index, operation -> operation.order to index + 1 }.toMap()
    val compiledOperations = operations.mapIndexed { index, operation ->
        operation.copy(
            order = index + 1,
