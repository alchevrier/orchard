...
An organization profile may declare external **policy sources**, including a separately administered Git repository containing engineering standards, security controls, architecture rules, approved components, domain assurance packs, onboarding obligations, and operating procedures. Orchard records an allowlisted repository identity, trust roots or signature requirements, permitted refs, minimum or exact revision, freshness limit, required paths, schema versions, precedence, and the authority allowed to update that source declaration. A branch name or remote default branch is discovery input, not sufficient authority; each effective profile pins the verified commit and content hashes it actually compiled.

Required policy sources are onboarding gates. Before a human user, service identity, agent, project repository, or workspace can become active under the organization profile, Orchard must fetch or synchronize every mandatory source through the configured credential boundary, verify repository identity and revision authenticity, validate required content, compile the effective policy, and record an onboarding attestation tied to the subject, organization-profile revision, source commits, content hashes, and time. Missing credentials, unavailable mandatory sources, rejected signatures, stale revisions, malformed policy, or unresolved precedence conflicts fail closed. Configured break-glass or offline operation requires an explicit, time-bounded waiver with a maximum accepted revision age and compensating controls; it is never inferred from a failed fetch.

RAG over an organizational policy repository is a derived navigation and retrieval layer, not policy authority. Orchard builds or obtains the index only from verified, source-pinned content and binds every retrieved passage to its repository, commit, path, range or structured record, and content hash. Retrieval ranking, embeddings, summaries, and model answers cannot add, weaken, or override policy. Deterministic policy compilation and exact source inspection decide applicability and conflicts; the RAG projection may help agents and people find relevant rules. An index with unknown provenance, a mismatched source revision, or incomplete required coverage is discarded and rebuilt or causes onboarding and governed execution to remain blocked according to policy.

Policy sources may be open community repositories rather than private organizational assets. An open policy commons can publish reusable engineering standards, domain assurance packs, research methods, approved-component catalogs, inspectors, and acceptance templates through normal version control and review. Orchard applies the same trust rules to public and private sources: community popularity, retrieval rank, or a model summary does not grant authority. An adopter explicitly allowlists and pins the source, verifies its manifest and revision, selects applicable packs, and may compose stricter private overlays without modifying or disclosing them upstream. Improvements can return through ordinary pull requests while each organization retains exact control over which revision and rules become effective authority.

Synchronization applies continuously after onboarding. Organization policy defines polling or event-driven refresh, maximum staleness, rollout windows, and whether a newly accepted source revision blocks new work immediately or permits existing pinned attempts to finish. Every execution contract records the effective source revisions it inherited. Updating the remote repository never retroactively changes an in-flight contract; Orchard admits the new policy revision, performs impact analysis, and creates explicit migration, re-attestation, remediation, or suspension decisions.
...
- External organizational policy sources must be allowlisted, authenticated, source-pinned, freshness-checked, deterministically compiled, and attested before covered subjects can complete onboarding or governed execution.
- Organizational RAG indexes are disposable source-bound projections; retrieval output cannot become policy authority or conceal missing, stale, or conflicting source material.
